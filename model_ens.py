...
risk_hosp_F = np.array([2.8, 1.1, 2.3, 3.7, 4.4, 6.6, 13.8, 27.5, 28.5]) /100./1.4
risk_hosp_M = np.array([3.5, 1.2, 2.1, 4.0, 7.3, 12.1, 22.6, 38.4, 44.3])/100./1.4

risk_hosp_severe = np.array([0.1, 0.1, 0.1, 0.2, 0.6, 1.3, 3.5, 5.2, 2.9])/100.
risk_hosp_severe_F = np.array([0.1, 0.04, 0.1, 0.2, 0.3, 0.6, 1.9, 3.4, 2.1])/100./1.1
risk_hosp_severe_M = np.array([0.1, 0.1, 0.1, 0.3, 0.9, 2.1, 4.9, 6.9, 4.3])/100./1.1
...
risk_cfr_F = np.array([0,0,0,0.1,0.1,0.3,1.8,8.1,19])/100./1.3
risk_cfr_M = np.array([0,0,0,0.1,0.3,1.1,4.6,15.3,30.7])/100./1.3
...
for ens in range(Nens):
    ...
    dayend = day0 + datetime.timedelta(days=int(N+fcs_length))
    day = day0
    t= 0
    while day < dayend:
        ...
        for i in  range(9):
            for k in range(int(np.round(female_daily[i,t]+np.random.uniform(-0.05,0.05),0))):
                ...
                outcome_hosp = np.random.choice([1,0],\
                                    p=[risk_hosp_F[i],1-risk_hosp_F[i]])
                outcome_icu = np.random.choice([1,0],\
                                    p=[risk_hosp_severe_F[i],1-risk_hosp_severe_F[i]])
                ...
                deaths[ens,t+fin_test2death] += np.random.choice([1,0],p=[risk_cfr_F[i],1-risk_cfr_F[i]])
...
plt.plot_date(dates[:N+fcs_length],np.cumsum(icu_in_avg-icu_out_avg)[:N+fcs_length],"r-",xdate=True,label="ICU model")
plt.plot_date(dates[:N+fcs_length],np.cumsum(hospitalised_in_avg-hospitalised_out_avg)[:N+fcs_length],"b-",xdate=True,label="HOSP model")
plt.plot_date(dates[:N+fcs_length],np.cumsum(deaths_avg)[:N+fcs_length],"k-",xdate=True,label="deaths model")
...
plt.plot_date(dates[:nn],hos_data,"bo",markersize=6,label="HOSP data",xdate=True)
...
plt.plot_date(dates[:N+fcs_length],icu_model*np.mean(icu_data[nn-5:nn])/np.mean(icu_model[nn-5:nn]),"r-",xdate=True,label="ICU model")
plt.plot_date(dates[:N+fcs_length],hosp_model*np.mean(hos_data[nn-5:nn])/np.mean(hosp_model[nn-5:nn]),"b-",xdate=True,label="HOSP model")
plt.plot_date(dates[:N+fcs_length],deaths_model*np.mean(cum_deaths_data[nn-5:nn])/np.mean(deaths_model[nn-5:nn]),"k-",xdate=True,label="deaths model")
